...
func CheckOrbitList() (tests []MovementTest, err error) {
	...
	for n, actual := range actualList {
		...
		entry.AssertionResult.A = InDelta(allowedDeltaA, "A")
		entry.AssertionResult.Z_avg = InDelta(allowedDeltaZ_avg, "Z_avg")
		entry.AssertionResult.Delta = InDelta(allowedDeltaDelta, "Delta")
		entry.AssertionResult.Alpha = InDelta(allowedDeltaAlpha, "Alpha")
		entry.AssertionResult.Beta = InDelta(allowedDeltaBeta, "Beta")
		entry.AssertionResult.Lambda = InDelta(allowedDeltaLambda, "Lambda")
		entry.AssertionResult.Lambda_deriv = InDelta(allowedDeltaLambdaDer, "Lambda_deriv")
		entry.AssertionResult.Beta_deriv = InDelta(allowedDeltaBetaDer, "Beta_deriv")
		entry.AssertionResult.Inc = InDelta(allowedDeltaInc, "Inc")
		entry.AssertionResult.Wmega = InDelta(allowedDeltaWmega, "Wmega")
		entry.AssertionResult.Omega = InDelta(allowedDeltaOmega, "Omega")
		entry.AssertionResult.V_g = InDelta(allowedDeltaVg, "V_g")
		entry.AssertionResult.V_h = InDelta(allowedDeltaVh, "V_h")
		entry.AssertionResult.Axis = InDelta(allowedDeltaAxis, "Axis")
		entry.AssertionResult.Exc = InDelta(allowedDeltaExc, "Exc")
		entry.AssertionResult.Nu = InDelta(allowedDeltaNu, "Nu")
		entry.AssertionResult.E_apex = InDelta(allowedDeltaEapex, "E_apex")
		entry.AssertionResult.E_deriv = InDelta(allowedDeltaEderiv, "E_deriv")
		tests[n] = entry
	}
	stop()
